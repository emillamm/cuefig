...
#ObjectRef: {
	...
	scaleDown?: string @go(ScaleDown) @protobuf(4,bytes,opt)
}
...
#ALBTrafficRouting: {
	...
	// +optional
	ingresses?: [...string] @go(Ingresses,[]string) @protobuf(6,bytes,opt)
}
...
#RolloutTrafficRouting: {
	...
	maxTrafficWeight?: null | int32 @go(MaxTrafficWeight,*int32) @protobuf(11,varint,opt)
}
...
#NginxTrafficRouting: {
	...
	// +optional
	canaryIngressAnnotations?: {[string]: string} @go(CanaryIngressAnnotations,map[string]string) @protobuf(5,bytes,rep)
}
...
#RolloutExperimentStep: {
	...
	// +optional
	analysisRunMetadata?: #AnalysisRunMetadata @go(AnalysisRunMetadata) @protobuf(5,bytes,opt)
}
...
#CanaryStep: {
	...
	plugin?: null | #PluginStep @go(Plugin,*PluginStep) @protobuf(9,bytes,opt)
}
...
#PluginStep: {
	...
	config?: _ @go(Config,json.RawMessage) @protobuf(2,bytes,opt)
}
...
#RolloutAnalysis: {
	...
	// +patchStrategy=merge
	templates?: [...#AnalysisTemplateRef] @go(Templates,[]AnalysisTemplateRef) @protobuf(1,bytes,rep)
...
#RolloutStatus: {
	...
	albs?: [...#ALBStatus] @go(ALBs,[]ALBStatus) @protobuf(26,bytes,opt)
}
...
#CanaryStatus: {
	...
	stepPluginStatuses?: [...#StepPluginStatus] @go(StepPluginStatuses,[]StepPluginStatus) @protobuf(6,bytes,rep)
}
...
#StepPluginStatus: {
	...
	index: int32 @go(Index) @protobuf(1,bytes)
	...
	operation: #StepPluginOperation @go(Operation) @protobuf(3,bytes,casttype=StepPluginOperation)

	// Phase is the resulting phase of the operation
	phase?: #StepPluginPhase @go(Phase) @protobuf(4,bytes,opt,casttype=StepPluginPhase)
	...
	startedAt?: null | metav1.#Time @go(StartedAt,*metav1.Time) @protobuf(6,bytes)

	// UpdatedAt indicates when the plugin was last called for the operation
	updatedAt?: null | metav1.#Time @go(UpdatedAt,*metav1.Time) @protobuf(7,bytes,opt)

	// FinishedAt indicates when the operation was completed
	finishedAt?: null | metav1.#Time @go(FinishedAt,*metav1.Time) @protobuf(8,bytes,opt)

	// Backoff is a duration to wait before trying to execute the operation again if it was not completed
	backoff?: #DurationString @go(Backoff) @protobuf(9,bytes,opt,casttype=DurationString)

	// Executions is the number of time the operation was executed
	executions?: int32 @go(Executions) @protobuf(10,varint,opt)

	// Disabled indicates if the plugin is globally disabled
	disabled?: bool @go(Disabled) @protobuf(11,bytes,opt)

	// +kubebuilder:validation:Schemaless
	// +kubebuilder:pruning:PreserveUnknownFields
	// +kubebuilder:validation:Type=object
	// Status holds the internal status of the plugin for this operation
	status?: _ @go(Status,json.RawMessage) @protobuf(12,bytes,opt)
}

// StepPluginPhase is the overall phase of a StepPlugin
#StepPluginPhase: string // #enumStepPluginPhase

#enumStepPluginPhase:
	#StepPluginPhaseRunning |
	#StepPluginPhaseSuccessful |
	#StepPluginPhaseFailed |
	#StepPluginPhaseError
...
#StepPluginOperation: string // #enumStepPluginOperation

#enumStepPluginOperation:
	#StepPluginOperationRun |
	#StepPluginOperationTerminate |
	#StepPluginOperationAbort
...
#ALBStatus: {
	...
	ingress?:           string          @go(Ingress) @protobuf(4,bytes,opt)
}
...
#ScaleDownNever:         "never"
#ScaleDownOnSuccess:     "onsuccess"
#ScaleDownProgressively: "progressively"
